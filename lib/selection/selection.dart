...
/**
 * Callback to access key value from a given data object. During the
 * process of binding data to Elements, the key values are used to
 * match Elements that have previously bound data
 */
typedef KeyFunction(datum);
...
abstract class Selection {
  ...
  /**
   * Scope of this selection that manages the element, data associations
   * for all elements in this selection (and the sub-selections)
   */
   ...
   *
   * If an element does not have a matching decendant, a placeholder
   * is used in it's position - thus being able to match the indices
   * of elements in the current and the created sub-selection.
   *
   * Any data bound to elements in this selection is inherited by
   * the selected decendants.
   */
  Selection select(String selector);

  /**
   * Same as [select], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the selected element
   * that will be selected.
   */
  Selection selectWithCallback(ChartedCallback<Element> fn);

  /**
   * Creates and returns a new [Selection] containing all elements
   * matching [selector] under each element in the current selection.
   *
   * The resulting [Selection] is nested with elements from current
   * selection as parents and the selected decendants grouped by
   * elements in the current selection.  When no decendants match
   * the selector, the collection of selected elements in a group is empty.
   *
  ...
  /**
   * Same as [selectAll], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get a collection of
   * selected elements that will be part of the new selection.
   */
  ...
  /**
   * Same as [classed], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the boolean
   * value that indicates if the class must be added or removed.
   */
  ...
  /**
   * Same as [style], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the value of
   * the property.
   */
   ...
   *
   * Returns a [Selection] containing the newly created elements.
   * As with [select], any data bound to the elements in this selection
   * is inherited by the new elements.
   */
  Selection append(String tag);

  /**
   * Same as [append], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the element
   * to be appended.
   */
  Selection appendWithCallback(ChartedCallback<Element> fn);

  /**
   * Inserts a child node to each element in the selection before the
   * first element matching [before] or before the element returned by
   * [beforeFn].
   *
   * Returns a [Selection] containing the newly created elements.
   * As with [select], any data bound to the elements in this selection
  ...
  /**
   * Same as [insert], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the element
   * to be inserted.
   */
  ...
  /**
   * Same as [data], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the data
   * to be set on the current element.
   */
  DataSelection dataWithCallback(
      ChartedCallback<Iterable> fn, [KeyFunction keyFn]);

  /**
   * Associates data with all the elements - no join is performed
   * Unlike [data], this does not compute the enter, update
   * and exit selections.
   */
  void datum(Iterable vals);

  /**
   * Same as [datum], but calls [fn] for each non-null element in
   * the selection (with data associated to the element, index of the
   * element in it's group and the element itself) to get the data
   * to be set on the current element.
   */
  ...
  /**
   * Starts a transition for the current selection. Transitions behave much like
   * selections, except operators animate smoothly over time rather than
...
/**
 * [EnterSelection] is a sub-selection that represents missing elements
 * of a selection - an element is considered missing when there is data
 * and no corresponding element in a selection.
 */
...
abstract class DataSelection extends Selection {
  ...
  /**
   * A view of the current selection containing elements that don't
